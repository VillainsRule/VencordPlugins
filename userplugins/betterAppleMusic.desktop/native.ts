...
(async () => {
    ...
    if (!scriptURL) return console.log('fuck, apple music patched it [err 1]');
    ...
    if (!authExtract) return console.log('fuck, apple music patched it [err 2]');
...
async function fetchRemoteData({ id, name, artist, album }: { id: string, name: string, artist: string, album: string; }) {
    ...
    try {
        ...
        console.error("[BetterAM] Failed to fetch remote data:", e);
        cachedRemoteData = {
            id,
